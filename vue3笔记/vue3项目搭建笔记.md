...
1.安装prettier(脚手架本身自带可以不用安装)

```shell
npm install prettier -D
```

2.配置.prettierrc文件：创建一个.prettierrc
...
接下来，我们需要去完成一个操作：在进行commit时，执行lint脚本：lint是自带的就是执行的时候会修复代码格式风格
...
* 第二步选择本次修改的范围（作用域）(这次修改影响的范围或者模块,比如这次你修改的是login模块,可以写个login)
...
我们也可以在scripts中构建一个命令来执行 cz：**npx cz 大家可能会用的不习惯,可以在package.json配置成 npm run commit**
...
3.使用husky生成commit-msg文件，验证提交信息：(可能不行,但是可以手动创建)
...
## 四. 项目搭建cli
![](img/11.30/微信截图_20220228182949.png)
### 4.1 eslint关闭
![](img/11.30/微信截图_20220228202751.png)
![](img/11.30/微信截图_20220228202851.png)
### 4.2 路由的配置
![](img/11.30/微信截图_20220228203708.png)
import 后面有个type说明这是一个TS的类型 (可写可不写,主要是为了声明,有区分度,说明我不是函数也不是组件)
这个类型是VUE-ROUTER自带的