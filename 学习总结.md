...
//=>JSON.stringify也可以扁平化数组(这个方法明显和上个一样,但还多加了一个正则)
...
#### 1、VUE全家桶
![](学习总结图片/6.3/1622716379(1).jpg)

#### 2、MVVM理解
**其实就是相当于VUE是VM(中间层),来处理视图层(div)和数据层(js中的数据)**
![](学习总结图片/6.3/1622721693(1).jpg)

![](学习总结图片/6.3/1622721729.jpg)

#### 3、比较VUE和操作DOM(来理解VUE)
普通操作DOM来改变
![](学习总结图片/6.3/1622721834(1).jpg)

使用vue创建的实例
相当于new了一个vue的实例出来,然后我们传对应的参数给vue这个类,和昨天学习的自己封装组件传的参数类似理解起来
一个页面可以创建多了实例,你可以每个在创建个div然后把ID传给新的new Vue来
![](学习总结图片/6.3/1622721912(1).jpg)
![](学习总结图片/6.3/1622721937(1).jpg)

vue中的v-model
![](学习总结图片/6.3/1622722141(1).jpg)

#### 4、new Vue中的el 和data修改值的问题

**el不能直接获取HTML和body**
![](学习总结图片/6.3/微信截图_20210603201633.png)

![](学习总结图片/6.3/微信截图_20210603204241.png)
**对象假如添加新的键值对,视图不会重新渲染,除非直接覆盖原来的或者用vue内置的$set(obj,key,value)**
![](学习总结图片/6.3/微信截图_20210603204423.png)

**数组也和对象类似,不过他还有数组本身的push等方法可以修改,arr[2]='你好'这种是不行的**
![](学习总结图片/6.3/微信截图_20210603204832.png)

new Vue 里面的data和method都会挂在到实例上
![](学习总结图片/6.3/微信截图_20210603214052.png)
![](学习总结图片/6.3/微信截图_20210603214141.png)

#### 5、vue中一些常用指令
v-model
![](学习总结图片/6.3/微信截图_20210603215826.png)

v-once 就是不准备修改的数据,获取到第一次不打算改变了(用的不多,但是肯定会有他出现的场景)
![](学习总结图片/6.3/1622728596(1).jpg)
v-if和v-show区别
![](学习总结图片/6.3/微信截图_20210603215350.png)
v-html和v-text 在vue没加载之前只是相当于给标签加上一个自定义属性,浏览器不会解析,胡子语法的话会出现{{msg}} 因为他是在标签文本里面的(浏览器从上而上执行代码解析到胡子语法的时候,vue.js还没被解析到)
![](学习总结图片/6.3/微信截图_20210603215941.png)
v-bind
![](学习总结图片/6.3/微信截图_20210603220011.png)

#### 6、各种循环的比较
for of
![](学习总结图片/6.3/微信截图_20210603224007.png)

for in
**会遍历对象原型上自定义的方法,所以要进行hasOwnProperty判断**
![](学习总结图片/6.3/微信截图_20210603224228.png)

#### 7、对象中函数的写法和箭头函数问题
![](学习总结图片/6.3/微信截图_20210603225156.png)
函数名(){}这个写法是ES6新语法,箭头函数的this是往外面找函数作用域所在的作用域,这里外面都是对象所以this是window
![](学习总结图片/6.3/1622731955(1).jpg)

#### 8、vue中指令
v-on
如果不传$event 默认是参数,传了第一个参数就是事件对象
![](学习总结图片/6.3/微信截图_20210603230303.png)
事件修饰符(vue的官方文档)
![](学习总结图片/6.3/微信截图_20210603231626.png)
![](学习总结图片/6.3/微信截图_20210603231642.png)
![](学习总结图片/6.3/微信截图_20210603231655.png)
![](学习总结图片/6.3/微信截图_20210603231712.png)
...
单选
![](学习总结图片/6.3/1622735235(1).jpg)



表单的多选和全选不全选案例
![](学习总结图片/6.3/1622734735(1).jpg)
![](学习总结图片/6.3/微信截图_20210603234422.png)
...
#### 1、过滤器
![](学习总结图片/6.6/微信截图_20210606143709.png)
**也可以在v-xx里面写方法**
![](学习总结图片/6.6/微信截图_20210606143836.png)

![](学习总结图片/6.6/微信截图_20210606144424.png)
![](学习总结图片/6.6/微信截图_20210606144313.png)
...
![](学习总结图片/6.6/微信截图_20210606141508.png)
![](学习总结图片/6.6/1622959922(1).jpg)
...
vue中强制刷新视图
![](学习总结图片/6.6/微信截图_20210606141813.png)

**计算属性必须要关联响应式数据(data中的值才是响应式)**
![](学习总结图片/6.6/微信截图_20210606142043.png)

**计算属性完整写法get,set**

![](学习总结图片/6.6/微信截图_20210606143017.png)
...
![](学习总结图片/6.6/微信截图_20210606143017.png)
![](学习总结图片/6.6/微信截图_20210606161013.png)
![](学习总结图片/6.6/微信截图_20210606161109.png)
![](学习总结图片/6.6/微信截图_20210606161210.png)

#### 4、监听器

![](学习总结图片/6.6/微信截图_20210606162730.png)
...
#### 5、Class 与 Style 绑定
class语法
![](学习总结图片/6.6/微信截图_20210606165054.png)

可以用计算属性
![](学习总结图片/6.6/微信截图_20210606165128.png)

数组语法
![](学习总结图片/6.6/微信截图_20210606165156.png)


style语法

![](学习总结图片/6.6/微信截图_20210606165311.png)
![](学习总结图片/6.6/微信截图_20210606165323.png)

#### 6、数组reduce方法
把数组里面的数组从左到右累加起来
第一个参数是回调函数,第二个参数是累加的初始值(可写,可不写)
![](学习总结图片/6.6/微信截图_20210606184453.png)
![](学习总结图片/6.6/微信截图_20210606184653.png)

#### 7、生命周期函数

![](学习总结图片/6.6/lifecycle.png)
![](学习总结图片/6.6/20200304231954749.png)

一般都直接写el 不用$mount
![](学习总结图片/6.6/微信截图_20210606200548.png)

如果写了template 那就会渲染template不渲染el里面的东西了
![](学习总结图片/6.6/微信截图_20210606200806.png)
...
![](学习总结图片/6.7/微信截图_20210607190422.png)
...
![](学习总结图片/6.7/微信截图_20210607195724.png)
...
但是你可以在template中使用
![](学习总结图片/6.7/微信截图_20210607200318.png)
![](学习总结图片/6.7/微信截图_20210607200644.png)
![](学习总结图片/6.7/Inked微信截图_20210607194532_LI.jpg)

组件的template
![](学习总结图片/6.7/微信截图_20210607215208.png)

把template都抽离出来
![](学习总结图片/6.7/微信截图_20210607215208.png)
![](学习总结图片/6.7/微信截图_20210607215259.png)

组件的插槽
![](学习总结图片/6.7/微信截图_20210607220624.png)

具名插槽
![](学习总结图片/6.7/微信截图_20210607220855.png)

作用域插槽
![](学习总结图片/6.7/微信截图_20210607221225.png)

局部组件
![](学习总结图片/6.7/微信截图_20210607224655.png)
...
![](学习总结图片/6.8/微信截图_20210608210305.png)

props命名规范
![](学习总结图片/6.8/微信截图_20210608210147.png)

props格式验证

![](学习总结图片/6.8/微信截图_20210608210422.png)
![](学习总结图片/6.8/微信截图_20210608210736.png)
![](学习总结图片/6.8/微信截图_20210608210821.png)
自定义规则例子2
![](学习总结图片/6.8/微信截图_20210608210905.png)

组件上可以直接写class style  这就解释了项目中为什么可以改别人写好组件样式或者覆盖的原因
![](学习总结图片/6.8/微信截图_20210608211005.png)

尽量不要子组件修改父组件传过来的props
![](学习总结图片/6.8/微信截图_20210608211259.png)

虽然修改了但是会报错控制台
![](学习总结图片/6.8/微信截图_20210608211358.png)

解释单项数据流
![](学习总结图片/6.8/微信截图_20210608212232.png)
![](学习总结图片/6.8/微信截图_20210608212339.png)
![](学习总结图片/6.8/微信截图_20210608212432.png)

子组件修改父组件信息$emit
![](学习总结图片/6.8/微信截图_20210608214107.png)
@kobe-bryant支持,不支持@kobeBryant这种写法
![](学习总结图片/6.8/微信截图_20210608214107.png)
![](学习总结图片/6.8/微信截图_20210608214319.png)
![](学习总结图片/6.8/微信截图_20210608214348.png)


#### 2、兄弟组件通信

事件总线bus
![](学习总结图片/6.8/微信截图_20210608224057.png)
![](学习总结图片/6.8/微信截图_20210608224113.png)
![](学习总结图片/6.8/微信截图_20210608224122.png)
![](学习总结图片/6.8/微信截图_20210608224139.png)
...
<img src="学习总结图片/6.9/微信截图_20210610000504.png" style="zoom:50%;" />
<img src="学习总结图片/6.9/微信截图_20210610000610.png" style="zoom:50%;" />
...
<img src="学习总结图片/6.9/微信截图_20210610001607.png" style="zoom:33%;" />
<img src="学习总结图片/6.9/微信截图_20210610001720.png" style="zoom: 50%;" />

要打包同时启动服务设置命令
![](学习总结图片/6.9/微信截图_20210610001903.png)
<img src="学习总结图片/6.9/微信截图_20210610002115.png" style="zoom:50%;" />

打包HTML
<img src="学习总结图片/6.9/微信截图_20210610002321.png" style="zoom:50%;" />
<img src="学习总结图片/6.9/微信截图_20210610002241.png" style="zoom:50%;" />
<img src="学习总结图片/6.9/微信截图_20210610002208.png" style="zoom:50%;" />
<img src="学习总结图片/6.9/微信截图_20210610002343.png" style="zoom:50%;" />

项目中可能会有浏览器缓存所以需要每次加载不同的
<img src="学习总结图片/6.9/微信截图_20210610002804.png" style="zoom:50%;" />
<img src="学习总结图片/6.9/微信截图_20210610002612.png" style="zoom:50%;" />
<img src="学习总结图片/6.9/1623256119(1).jpg" style="zoom:50%;" />
