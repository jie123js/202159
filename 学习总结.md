...
![微信截图_20210915201857](C:\Users\郑大牛\Desktop\学习笔记\学习总结图片\6.6\微信截图_20210915201857.png)
...
#### 3、git add后想删除
git rm --cached 文件名
这样git add的文件就会不add了 但是工作区(也就是我们的磁盘)是不会被删除的 只是从暂存区删除

#### 4、commit后查看提交信息
![](学习总结图片/7.11/微信截图_20210711133122.png)

#### 5、版本穿梭
先用 git reflog  或者 git log查看提交信息
接着复制前面的版本号  git reset --hard 版本号
![](学习总结图片/7.11/微信截图_20210711134735.png)


#### 6、分支

![](学习总结图片/7.11/微信截图_20210711141943.png)

解决合并冲突
![](学习总结图片/7.11/微信截图_20210711142448.png)

直接把要的代码保留下来,删除特殊符合即可  然后再add  commit
![](学习总结图片/7.11/微信截图_20210711142532.png)


#### 7、远程仓库
![](学习总结图片/7.11/微信截图_20210711154650.png)
 -u 就是记住本次推送的信息，下次就不用写推送信息了，可以直接 git push
git push -u origin master

## 六十五、7.12(学习总结)
今天开始进行为期2年的阿里之路  如果中间被当前公司淘汰了 我们也要虚心接受 不能怨天尤人 加油吧  阿杰
第一天上好班 压力真的好大啊  感觉随时会被炒鱿鱼  一点安全感都没有  没有劳动合同
#### 1、头条项目第一天总结
主要笔记在 [](https://www.yuque.com/lipengzhou/toutiao-publish-vue/dkhzid)
创建脚手架项目

小细节补充
 Local:   http://localhost:8080/
  - Network: http://192.168.10.216:8080/
这个Network是连接无线的话 手机啊 其他人电脑啊 都可以访问的
[](https://www.yuque.com/lipengzhou/toutiao-publish-vue/dkhzid)

@在vue-cli当中表示src目录下的
![](学习总结图片/7.12/微信截图_20210713001212.png)
...
​	## 六十六、7.13(学习总结)
先黑马PC端项目弄好(15天左右) 移动端(15天左右) 尚硅谷(PC端15天左右) 这里结束差不多9月初了
然后再去研究开课吧VUE差不多起步15天把
前端进阶  node.js  webpack  vue3 vite  ts 正则表达式

项目收获
#### 1、axios 不同基准路径代理请求方式
![](学习总结图片/7.13/微信截图_20210713215501.png)


#### 2、element from表单验证 支持正则 写法如下
![](学习总结图片/7.13/微信截图_20210713222029.png)


#### 3、data()里面的var
data()里面定义的值 可以用var声明
![](学习总结图片/7.13/微信截图_20210713224152.png)
(这也是插件的自定义验证规则内容)

#### 4、axios复用
axios在request中封装好后  由于很多接口要复用所以我们设置一个api文件夹专门来存放接口 复用
![](学习总结图片/7.13/微信截图_20210714000033.png)

#### 5、路由配置规范
路由如果有默认子路由可以不设置name
![](学习总结图片/7.13/微信截图_20210714004203.png)

## 六十七、7.14(学习总结)

#### 1、iconfont在线symbol 用法
先在根index里面引入在线JS
![](学习总结图片/7.14/微信截图_20210714221607.png)

再把样式写在css中
![微信截图_20210714221710](C:\Users\郑大牛\Desktop\学习笔记\学习总结图片\7.14\微信截图_20210714221710.png)

如果是font 在线使用的话
是用 link标签引入

#### 2、img在线链接打不开
img在线链接打不开   添加  <meta name="referrer" content="no-referrer"> 即可

#### 3、axios中请求头
默认会配'content-type'我们可以不用写 
假如接口有其他要求在headers里面我们再写  比如这里的Authorization

![](学习总结图片/7.14/微信截图_20210714230005.png)


#### 4、使用localStorage来保存用户登录信息
因为只能保存字符串(所以会默认把保存的数据转成字符串 对象转成字符串是[object object])所以我们要转成JSON格式字符串来保存
<img src="学习总结图片/7.14/微信截图_20210714233458.png" style="zoom:50%;" />
![](学习总结图片/7.14/微信截图_20210714233510.png)

要使用得转回对象JSON.parse()
<img src="学习总结图片/7.14/微信截图_20210714233740.png" style="zoom:50%;" />
<img src="学习总结图片/7.14/微信截图_20210714233840.png" style="zoom:50%;" />
 window.localStorage.removeItem('user')这是删除
#### 5、axios请求拦截器
这里因为很多请求都需要一个headers里面的Authorization 信息  所以我们统一设置
<img src="学习总结图片/7.14/微信截图_20210714235719.png" style="zoom:50%;" />
注意 这个request本来应该是axios的 但是我们前面let request = axios.crea({})创建了 所以这里用request 如果你没这样做 要用axios.interceors.request.use
...
#### 3、一些组件不支持vue原生语法
使用.native
![](学习总结图片/7.15/微信截图_20210715230710.png)


#### 4、关于后台接口需要参数的传值问题
 1. Body 参数使用 data 设置
 2. Query 参数使用 params 设置
 3. Headers 参数使用 headers 设置
...
#### 5、关于布局可以多用饿了么card布局
样式如下  一小块 一小块的
![](学习总结图片/7.15/微信截图_20210716002023.png)


#### 6、代码冗长优化(可以学习)
这里好多if判断太多了  优化一下
![](学习总结图片/7.15/微信截图_20210716004245.png)

这里status在当前项目是没用的.只是写了而已  不要在意
![](学习总结图片/7.15/微信截图_20210716004354.png)


## 六十九、7.16(学习总结)
#### 1、promise.all其中一个失败,怎么让其他的也都执行
给all里面每个都增加一个catch事件
首先我们需要知道Promise的状态具有可传递性，其次catch方法在执行后也会返回一个状态为resolved的新Promise实例，
所以我们只要将可能reject的Promise实例先catch一遍就可以了，就像做一次状态预加工：
...
```
promise.catch的深入理解
catch与 then 方法相同，执行后会 return 一个新的 Promise 对象，以实现链式调用，你可以显式的 return 一个 Promise 对象，若不显式的 return 则会 return 一个以 undefined 值 resolve 的 Promise 对象
...
#### 2、胡子语法数据没有得到,怎么等数据得到再渲染
v-cloak
![](学习总结图片/7.15/微信截图_20210716180952.png)


## 七十、7.17-7.18周末(学习总结)

#### 1、本地图片的渲染
不能用三点运算符,因为本地图片必须要在打包的时候就存在,如果三点是不会被渲染的  需要用v-if
![](学习总结图片/7.18/微信截图_20210718123630.png)
...
#### 3、element组件button一个坑
 el-button组件中@click事件如果你不传参他会默认传一个{istrue:true}  如果你绑定的事件需要传参 记住自己要加上参数


#### 4、数组数据的处理
this.result[0] 这种数据如果result为空就会报错
所以我们需要做一个非空的验证
this.result?this.result[0]:null

#### 5、分页点击问题
如果用户还没加载就疯狂点下一页肯定会出现问题
所以我们需要在用户点击发送请求的时间禁用分页的点击
element中的分页有一个disable属性
最好每个点击的按钮 都在加载数据的时候禁用  比如提交  登录啊

#### 6、接口请求中出现的路径参数
我们需要传过来拼接上去
![](学习总结图片/7.18/微信截图_20210718152338.png)

#### 7、JS中数字有最大值超出的话需要特殊处理
...
这个API可以自己配置axios返回的数据的处理  默认axios是把返回的数据转成它会在内部使用 JSON.parse() 把后端返回的数据转为 JavaScript 对象。
我们需要用插件来转
...
#### 8、使用删除操作的时候
删除后需要更新视图 但是我们不知道我们是在那一页更新视图的
element page组件中有一个 current-page.sync 可以拿到当前页数
这样我们更新视图就可以直接拿这个数据去获取

## 七十一、7.19学习总结
今天没啥收获 就是工作上疯狂研究echarts  晚上回来写写项目 没学到新知识
#### 1、git回退分支
![](学习总结图片/7.19/微信截图_20210719195451.png)


#### 2、多层解构赋值
![](学习总结图片/7.19/微信截图_20210719202100.png)

## 七十二、7.20-7.21(学习总结)
**学习资源总结**
黑马和尚硅谷基础的前端视频 全套 各1套
爪哇 和 开课吧 进阶视频各一套
#### 1、富文本编辑器
 如果要深度定制用 tip-tap
 普通使用用element-tiptap
 ...
 #### 2、关于图片上传问题(针对element-tiptap)
主要是我们要会用别人的插件  看懂文档
如果图片上传保存的是base64的话 会超级长
我们可以使用后端上传图片的接口返回的地址这样变成短的
![](学习总结图片/7.21/微信截图_20210721205433.png)
图片注释  image是这个接口后端要的字段名字  
new FormData 里面append添加的数据是后端接口需要的file类型数据

#### 3、element响应式布局layout
布局可以多用这个layout
![](学习总结图片/7.21/微信截图_20210721220517.png)

#### 4、渲染小技巧
如果你的对话框或者弹出层点击关闭 里面渲染的内容还没被删除
可以设置v-if 这样根据true和false的改变就会直接重新渲染
![](学习总结图片/7.21/微信截图_20210721232833.png)


#### 5、element上传组件的使用
图片同上
后端需要的参数名
![](学习总结图片/7.21/QQ截图20210722002143.png)
...
应该禁用按钮,防止用户一直点
element的button组件有
element 也提供了v-loading所有组件都可以用

#### 2、遍历出来的数据绑定同一个数据
会出现状态同步.处理方法可以给对象绑定一个自己的值

#### 3、数组里面数据想要响应式变化
赋值前就需要把数据先定义,不然再赋值是不会响应式的
法一用$set
![](学习总结图片/7.21/微信截图_20210722224611.png)
法二data里面的数据赋值的时候就需要提前把数据添加好
![](学习总结图片/7.21/微信截图_20210722224938.png)

## 七十四、7.23学习总结
#### 1、PC端打开文件夹且预览

input设置成file时候点击的时候就可以打开文件夹,但是这个样式太丑了,一般大家都不用
![](学习总结图片/7.21/微信截图_20210723154856.png)
一般我们都 hidden 然后通过其他方式打开 比如设置ref,这种方式
![](学习总结图片/7.21/微信截图_20210723154957.png)

也可以配合label标签 但是需要名字对应(for和id),然后在label标签里面放上自己喜欢的东西
![](学习总结图片/7.21/微信截图_20210723161700.png)

input本身带change事件,当选择的照片发生改变的时候会触发,但是点击同一张照片就不会触发了,
所以我们可以在事件里面每次选择结束后重置
![](学习总结图片/7.21/微信截图_20210723162547.png)

想要获取input点击后的数据需要用input这个节点的files属性
例如
files[0]里面就是这个选中图片的所有数据
![](学习总结图片/7.21/微信截图_20210723163959.png)
![](学习总结图片/7.21/微信截图_20210723163824.png)

使用这个方法可以创建一个新的URL 对象表示指定的 File 对象或 Blob 对象。
![](学习总结图片/7.21/微信截图_20210723164704.png)
 window.URL.createObjectURL(file.files[0])
 这个blob就是所创建的
 ![](学习总结图片/7.21/微信截图_20210723164631.png)


 ## 七十五、7.24-7.26(周末加周一学习,这周末玩了没学)

#### 1、自定义ESlint
如果要关闭验证,复制错误的名称,在这配置,具体可以查看ESlint官方
![](学习总结图片/7.26/微信截图_20210726143909.png)
![](学习总结图片/7.26/微信截图_20210726143649.png)
最好区分一下环境 开发和生产环境
![](学习总结图片/7.26/微信截图_20210726144350.png)

#### 2、vue-cli中一些配置没生效的话
![](学习总结图片/7.26/微信截图_20210726145403.png)
![](学习总结图片/7.26/微信截图_20210726145439.png)

#### 3、vue使用裁剪插件的方法步骤

地址
[](https://github.com/fengyuanchen/cropperjs#table-of-contents)
...
然后给要裁剪的img整一个div包起来
![](学习总结图片/7.26/微信截图_20210726164032.png)
...
```
但是你假如放在对话框里面,点击后才显示对话框的那种
你需要在对话框初始化以后再配置这个,不然是不会生效的
在vue中我们使用element插件的dio里面会有一个回调
![](学习总结图片/7.26/微信截图_20210726164750.png)
我们在这里面初始化
但是假如我们在这个函数里面初始化,其他函数就访问不到const cropper 这个属性了
我们可以在data中定义一个
cropper: null, // 裁切器示例

然后关闭再打开的话,还会是上一张的图片,所以我们需要在对话框关闭的回调中
销毁图片 ,有2种办法
![](学习总结图片/7.26/微信截图_20210726165108.png)
这2种插件自带的事件都可以使用,性能没什么区别
...
```
 const fd = new FormData()
  fd.append('photo', (这里是得到的blob或者file等类型的值))
  这个方法可以把得到的blob或者其他文件格式的文件变成后端要的字段,这里的photo是后端要的字段名字

#### 4、一些传值小技巧
如果传指定的值,可以结构一下,这样看的舒服一点
```JavaScript
onUpdateUser () {
      // 表单验证
      // 验证通过，提交表单
...
#### 1、input的文件上传再理解
![](学习总结图片/7.29/微信截图_20210729201153.png)

![](学习总结图片/7.29/微信截图_20210729201624.png)

#### 2、关于全局引入的脚本
![](学习总结图片/7.29/微信截图_20210729203537.png)
![](学习总结图片/7.29/微信截图_20210729203616.png)
![](学习总结图片/7.29/微信截图_20210729203616.png)

#### 3、关于遍历与V-for v-if问题
![](学习总结图片/7.29/微信截图_20210729205051.png)

#### 4、关于事件自带参数问题(比如父子组件通信)
自带参数,我们还想传参数的话,必须加一个$event 这个参数里面就是函数自带的参数
![](学习总结图片/7.29/微信截图_20210729225818.png)

#### 5、在组件中使用v-model
![](学习总结图片/7.29/微信截图_20210729233707.png)
然后子组件接收的时候值是value
![](学习总结图片/7.29/微信截图_20210729233911.png)
发送事件是input
![](学习总结图片/7.29/微信截图_20210729234029.png)

官方文档,其中change,value名字如何修改,在文档中这里说明
...
## 七十八、7.30-8.2(周末休息)
周末被很多事情弄的很糟心,而且周末的无所事事加重了我的烦恼,如果我能成为一个爱读书的人就好了

#### 1、响应拦截器,完整版
**响应拦截器想要跳转路由 必须要引入路由文件,只有在vue组件中使用是不需要导入的(因为vue里面进行了处理)**
import axios from 'axios'
**非组件模块可以这样加载使用 element 的 message 提示组件**
import { Message } from 'element-ui'
**要记住我们已经把axios重命名成request了(const request = axios.create) 所以一些axios开头的要改成request
完整版代码
```JavaScript
...
// })
```
## 七十九、8.3-8-12(天天玩游戏,玩物丧志)

## 八十、8.12
80天购买了拉钩教育大前端的课程 从今天开始提升自己,每天都要看!!!!!

#### 1、for循环和for in 循环 for of循环的区别
```JavaScript
...
![1631760096(1)](C:\Users\郑大牛\Desktop\学习笔记\学习总结图片\6.6\1631760096(1).jpg)